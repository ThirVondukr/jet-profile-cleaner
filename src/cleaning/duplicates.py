...
class ProfileCleaningResponse:
    ...
    def profile_changed(self) -> bool:
        return bool(
            self.removed_items_count
            or self.removed_orphan_items
            or self.duplicate_items
        )
...
def clean(profile: dict) -> ProfileCleaningResponse:
    ...
    ids_counter = collections.Counter(
        item["_id"] for item in profile["Inventory"]["items"]
    )
    duplicate_ids: Set[str] = {
        item_id for item_id, count in ids_counter.items() if count >= 2
    }
    ...
    items = {
        ...
        if item["_id"] not in duplicate_ids
    }
    response.removed_items_count += len(profile["Inventory"]["items"]) - len(items)

    # Set of root ids (Stash, quest items, equipment)
    inventory_root_items: Set[str] = {
        item for item in profile["Inventory"].values() if isinstance(item, str)
    }
    ...
    def is_orphan(item: dict):
        ...
        return (
            item["parentId"] not in items
            and item["parentId"] not in inventory_root_items
        )

    # Remove orphan items
    while True:
        items_size = len(items)
        response.removed_orphan_items.extend(
            item["_id"] for item in items.values() if is_orphan(item)
        )
        items = {
            item_id: item for item_id, item in items.items() if not is_orphan(item)
        }
